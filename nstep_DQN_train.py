...
class Game:
    ...
    def create_players(self,dqn):
        ...
        for i in range(self.num_DQN): # Add this
            self.players.append(nstep_DQN(f"DQN {i+1}", num, output_path=self.output_path))
        ...
        if len(self.players) > self.num_replace:
            ...
            for player in reaches[:self.num_replace]:
                if isinstance(player, CopyCat):
                    ...
                    new_players.append(nstep_DQN(f"DQN Player {self.num_DQN + 1}", num, output_path=self.output_path))
